...
def test_appbase_responses(
    ...
    response = requests_session.post(jefferson_url, json=request)
    response.raise_for_status()

    assert response.headers['Content-Type'] == 'application/json'
    assert 'x-jefferson-request-id' in response.headers
    if ':9000' not in jefferson_url:
    ...
    if isinstance(request, dict):
        ...
        assert dpayd_jrpc_response_validator(response_json) is None
        ...
        for item, expected in zip(response_json, expected):
            ...
            assert dpayd_jrpc_response_validator(item) is None
...
def test_response_headers(path, method, requests_session, jefferson_url):
    json_data = None
    if method == 'POST':
        json_data = jrpc_request
    resp = requests_session.request(method, jefferson_url + path, json=json_data)
...
@pytest.mark.live
def test_long_request_live(long_request, requests_session, jefferson_url,
                           dpayd_jrpc_response_validator,):
    response = requests_session.post(jefferson_url, json=long_request)
    response.raise_for_status()
    response_json = response.json()
    assert dpayd_jrpc_response_validator(response_json) is None
