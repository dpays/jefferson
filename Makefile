...
PROJECT_DOCKER_TAG := dpay/jefferson
...
run-local: ## run the python app without docker
	pipenv run python3 -m jefferson.serve  --server_workers=1 --upstream_config_file DEV_config.json
...
test-with-docker: Pipfile.lock build  ## run tests that depend on docker
	pipenv run pytest --rundocker --jeffersonurl http://localhost:8080
...
test-local-dpayd-calls:
	pipenv run pytest -vv tests/test_responses.py::test_dpayd_responses --jeffersonurl http://localhost:9000

.PHONY: test-local-appbase-calls
test-local-appbase-calls:
	pipenv run pytest -vv tests/test_responses.py::test_appbase_responses --jeffersonurl http://localhost:9000

.PHONY: test-local-appbase-translation-calls
test-local-appbase-translation-calls:
	pipenv run pytest -vv tests/test_responses.py::test_appbase_translation_responses --jeffersonurl http://localhost:9000

.PHONY: test-live-dev-appbase-calls
test-live-dev-appbase-calls:
	pipenv run pytest -vv tests/test_responses.py::test_appbase_responses --jeffersonurl https://greatchain.dpays.io
...
test-live-prod-appbase-calls:
	pipenv run pytest --maxfail=1 tests/test_responses.py::test_appbase_responses --jeffersonurl https://greatchain.dpays.io
...
%.pstats: perf
	-pipenv run python -m cProfile -o $*.pstats ./jefferson/serve.py --server_workers=1 --upstream_config_file DEV_config.json
...
install-python-dpay-macos: ## install dpay-python lib on macos using homebrew's openssl
	env LDFLAGS="-L$(shell brew --prefix openssl)/lib" CFLAGS="-I$(shell brew --prefix openssl)/include" pipenv install dpay
