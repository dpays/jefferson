...
from .errors import JeffersonCustomJsonOpLengthError
from .errors import JeffersonLimitsError
...
def is_valid_non_error_jefferson_response(
        request: JrpcRequest,
    ...
    try:
        ...
        if isinstance(request, list):
            return len(response) > 0 and \
                ...
                all(is_valid_non_error_jefferson_response(req, resp)
                    for req, resp in zip(request, response))
        return False
    except Exception as e:
        logger.error('is_valid_non_error_jefferson_response error', e=e,
                     jid=request.jefferson_request_id)
    return False


def is_get_block_request(request: JSONRPCRequest) -> bool:
    return request.urn.namespace in ('dpayd', 'appbase') and request.urn.method == 'get_block'


def is_get_block_header_request(request: JSONRPCRequest) -> bool:
    return request.urn.namespace in (
        'dpayd', 'appbase') and request.urn.method == 'get_block_header'


def is_get_dynamic_global_properties_request(request: JSONRPCRequest) -> bool:
    return request.urn.namespace in (
        'dpayd', 'appbase') and request.urn.method == 'get_dynamic_global_properties'
...
def limit_custom_json_op_length(ops: list, size_limit=None):
    if any(len(op[1]['json']) > size_limit for op in ops):
        raise JeffersonCustomJsonOpLengthError(size_limit=size_limit)
