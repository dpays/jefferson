...
class JeffersonInteralError(Exception):
    """Base class for errors that Jefferson logs, but don't
     result in JSONRPC error responses

    """
    message = 'Jefferson internal error'
    ...
    @property
    def jefferson_request_id(self) -> Optional[Union[str, int]]:
        try:
            return self.jsonrpc_request.jefferson_request_id
        except BaseException:
            pass
        try:
            return self.http_request.jefferson_request_id
        except BaseException:
            pass
        try:
            return self.http_request.headers['x-jefferson-request-id']
        except BaseException:
            pass
    ...
    def to_dict(self) -> dict:
        base_error = {
            ...
            'jefferson_request_id': self.jefferson_request_id
        }

        try:
            base_error.update(**self.kwargs)
        except Exception as e:
            logger.warning('JeffersonInteralError kwargs serialization error', e=e)
...
class JsonRpcError(JeffersonInteralError):
    ...
    def to_sanic_response(self) -> HTTPResponse:
        ...
        return response.json(error, headers={'x-jefferson-error-id': self.error_id})
...
class JeffersonLimitsError(JsonRpcError):
    code = 1700
    message = 'Request exceeded limit'


class JeffersonCustomJsonOpLengthError(JsonRpcError):
    code = 1800
